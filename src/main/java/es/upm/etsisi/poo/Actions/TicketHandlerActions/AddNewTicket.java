...
public class AddNewTicket extends Action {
    ...
    public int execute(String[] args) {
        ...
        if (args.length == 2) {
            // Formato: ticket new <cashId> <customerId>
            cashId = args[0];
            customerId = args[1];
            ticketId = null; // Se generará automáticamente en TicketBook
        } else if (args.length == 3) {
            // Formato: ticket new <ticketId> <cashId> <customerId>
            ticketId = args[0];
            cashId = args[1];
            customerId = args[2];
        } else {
        ...
        if (result == 0) {
            // Si el ID no se pasó, asumimos que TicketBook lo generó y lo devolvió implícitamente,
            // aunque para mostrarlo aquí, deberíamos haber modificado el retorno de addNewTicket.
            // Para mantener la firma actual (int addNewTicket), usaremos el ID pasado o una nota.
