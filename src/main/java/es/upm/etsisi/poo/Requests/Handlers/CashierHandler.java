...
public class CashierHandler extends RequestHandler {
    ...
    public CashierHandler() {
        super(handler_id);
        actions.put(AddUser.ID, new AddUser(App.getInstance().cashiers));
        actions.put(RemoveCashier.ID, new RemoveCashier());
        actions.put(ListUsers.ID, new ListUsers(App.getInstance().cashiers));
        actions.put(ListTicketsFromCashier.ID, new ListTicketsFromCashier());
    }
}
